	...
	for (u32 i=0; i<MaxTextureUnits; ++i)
	{
		...
		if (material.UseMipMaps && CurrentTexture[i]->hasMipMaps())
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
				...
				material.TextureLayer[i].BilinearFilter ? GL_NEAREST_MIPMAP_NEAREST :
			...
			f32 end, f32 density, bool pixelFog, bool rangeFog)
{
	...
	glFogf(GL_FOG_MODE, GLfloat((fogType==EFT_FOG_LINEAR)? GL_NEAREST : (fogType==EFT_FOG_EXP)?GL_EXP:GL_EXP2));
